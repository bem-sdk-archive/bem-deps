...
exports.default = function (deps, options) {
    if (!deps) {
        return [];
    }
...
function normalize(dep, options) {
    ...
    if (typeof dep === 'string' && options.parseString) {
        return options.parseString(dep);
    }
    ...
    if (typeof dep.elems === 'string' || _typeof(dep.elems) === 'object' && !Array.isArray(dep.elems)) {
        dep.elems = [dep.elems];
    }
